  ...
  return (
    <div className="screen fadein">
      ...
      <div className="campaigns-list">
        {campaigns.map(c => (
          <div className="campaign-card hover-effect" key={c.id}>
            ...
            <div className="campaign-stats">
              <span className="stat-item">
                <span className="stat-value">{c.stats.clicks}</span> кликов
              </span>
              <span className="stat-item">
                <span className="stat-value">{c.stats.budget}₽</span> бюджет
              </span>
            </div>
          </div>
        ))}
      ...
      <div className="section">
        <button className="btn btn-primary hover-effect" onClick={onShowUpload}>
          Загрузить креатив
        </button>
      </div>
    </div>
  ...
  return (
    <div className="screen fadein">
      ...
      <div className="creatives-grid">
        <div className="creative-card hover-effect">
          <div className="creative-preview"></div>
          <div className="creative-info">
            <span className="creative-name">Креатив 1</span>
            <span className="creative-stats">120 показов</span>
          </div>
        </div>
      </div>
      <div className="section">
        <button className="btn btn-primary hover-effect">Добавить креатив</button>
      </div>
    </div>
  );
}
      ...
      <div className="autopilot-card">
        <div className="autopilot-controls">
          ...
          <span className="autopilot-status">
            {autopilotEnabled ? 'Включен' : 'Выключен'}
          </span>
        </div>
        <div className="progress-bar">
          <div 
            className="progress-bar-inner" 
            style={{width: autopilotEnabled ? '100%' : '0%'}} 
          />
        </div>
      </div>
    </div>
  ...
  return (
    <div className="container">
        ...
        <div className="modal active">
          <div className="modal-content">
            <div className="modal-header">
              ...
              <button 
                className="close-btn hover-effect" 
            ...
            <button 
              className={`btn btn-primary hover-effect${uploadLoading ? ' loading' : ''}`}
      ...
      <nav className="bottom-nav">
        <div 
          className={`nav-item${activeTab === 0 ? ' active' : ''} hover-effect`}
        ...
        <div 
          className={`nav-item${activeTab === 1 ? ' active' : ''} hover-effect`}
          onClick={() => setActiveTab(1)}
        >
          <CreativesIcon className="nav-icon" />
          <span>Креативы</span>
        </div>
        <div 
          className={`nav-item${activeTab === 2 ? ' active' : ''} hover-effect`}
          onClick={() => setActiveTab(2)}
        >
          <AutopilotIcon className="nav-icon" />
          <span>Автопилот</span>
        </div>
      </nav>
    </div>
